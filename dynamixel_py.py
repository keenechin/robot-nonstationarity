...
class dxl():
    ...
    def open_port(self):
        ...
        self.group_desPos = dynamixel.groupSyncWrite(
            self.port_num, self.protocol, self.motor.ADDR_GOAL_POSITION, self.motor.LEN_GOAL_POSITION)
        self.group_desTor = dynamixel.groupSyncWrite(
            self.port_num, self.protocol, self.motor.ADDR_GOAL_TORQUE, self.motor.LEN_GOAL_TORQUE)  # NOTE: not all motors support them
        ...
        for m_id in self.motor_id:
            dxl_addparam_result = ctypes.c_ubyte(dynamixel.groupBulkReadAddParam(
                self.group_pos, m_id, self.motor.ADDR_PRESENT_POSITION, self.motor.LEN_PRESENT_POSITION)).value
        ...
        for m_id in self.motor_id:
            dxl_addparam_result = ctypes.c_ubyte(dynamixel.groupBulkReadAddParam(
                self.group_vel, m_id, self.motor.ADDR_PRESENT_VELOCITY, self.motor.LEN_PRESENT_VELOCITY)).value
            if dxl_addparam_result != 1:
                print("[ID:%03d] groupBulkRead addparam_vel failed" % (m_id))
                quit()

        # positions and velocities
        self.group_pos_vel = dynamixel.groupBulkRead(
            self.port_num, self.protocol)
        for m_id in self.motor_id:
            dxl_addparam_result = ctypes.c_ubyte(dynamixel.groupBulkReadAddParam(
                self.group_pos_vel, m_id, self.motor.ADDR_PRESENT_POS_VEL, self.motor.LEN_PRESENT_POS_VEL)).value
            if dxl_addparam_result != 1:
                print("[ID:%03d] groupBulkRead addparam_posfailed" % (m_id))
                quit()
    ...
    def okay(self, motor_id=None):
        ...
        if(dxl_comm_result != COMM_SUCCESS) or (dxl_error != COMM_SUCCESS):
            ...
            if dxl_error != COMM_SUCCESS:
                print("\n" + str(dynamixel.getRxPacketError(self.protocol,
                                                            dxl_error)) + ". Error_id:" + str(dxl_error))
        ...
        for dxl_id in motor_id:
            ...
            while(True):
                dynamixel.write1ByteTxRx(
                    self.port_num, self.protocol, dxl_id, self.motor.ADDR_TORQUE_ENABLE, enable)
                if(self.okay([dxl_id])):
                    break
                else:
                    print(
                        'dxl%d: Error with ADDR_TORQUE_ENABLE. Retrying after reboot ...' % dxl_id, flush=True)
    ...
    def get_pos_vel_old(self, motor_id):
        ...
        for i in range(self.n_motors):
            ...
            dxl_getdata_result = ctypes.c_ubyte(dynamixel.groupBulkReadIsAvailable(
                self.group_pos_vel, dxl_id, self.motor.ADDR_PRESENT_POS_VEL, self.motor.LEN_PRESENT_POS_VEL)).value
            if dxl_getdata_result != 1:
                ...
                dxl_present_position.append(dynamixel.groupBulkReadGetData(
                    self.group_pos_vel, dxl_id, self.motor.ADDR_PRESENT_POSITION, self.motor.LEN_PRESENT_POSITION))
                dxl_present_velocity.append(dynamixel.groupBulkReadGetData(
                    self.group_pos_vel, dxl_id, self.motor.ADDR_PRESENT_VELOCITY, self.motor.LEN_PRESENT_VELOCITY))
    ...
    def get_pos_vel(self, motor_id):
        ...
        if len(dxl_errored):
            self.okay(motor_id)
            print("groupBulkRead get_pos_vel failed. Sending last known values for dynamixel ids: " +
                  str(dxl_errored), flush=True)
    ...
    def getIndividual_vel(self, motor_id):
        ...
        for dxl_id in motor_id:
            dxl_present_velocity.append(dynamixel.read2ByteTxRx(
                self.port_num, self.protocol, dxl_id, self.motor.ADDR_PRESENT_VELOCITY))
        if (not self.okay(motor_id)):
            self.close(motor_id)
    ...
    def setIndividual_des_pos(self, motor_id, des_pos_inRadians):
        ...
        if(self.ctrl_mode == TORQUE_ENABLE):
            for dxl_id in motor_id:
                dynamixel.write1ByteTxRx(
                    self.port_num, self.protocol, dxl_id, self.motor.ADDR_TORQUE_CONTROL_MODE, TORQUE_DISABLE)
            if (not self.okay(motor_id)):
                self.close(motor_id)
        ...
        for i in range(len(motor_id)):
            dynamixel.write4ByteTxRx(self.port_num, self.protocol, motor_id[i], self.motor.ADDR_GOAL_POSITION, int(
                des_pos_inRadians[i]/POS_SCALE))
        if (not self.okay(motor_id)):
            self.close(motor_id)
    ...
    def set_des_pos(self, motor_id, des_pos_inRadians):
        ...
        for i in range(len(motor_id)):
            dxl_addparam_result = ctypes.c_ubyte(dynamixel.groupSyncWriteAddParam(
                self.group_desPos, motor_id[i], int(des_pos_inRadians[i]/POS_SCALE), self.motor.LEN_GOAL_POSITION)).value
            if dxl_addparam_result != 1:
                print(dxl_addparam_result)
                print("[ID:%03d] groupSyncWrite addparam failed" %
                      (motor_id[i]))
                self.close(motor_id)
                quit()
    ...
    def set_des_torque(self, motor_id, des_tor):
        ...
        if(self.ctrl_mode == TORQUE_DISABLE):
            for dxl_id in motor_id:
                dynamixel.write1ByteTxRx(
                    self.port_num, self.protocol, dxl_id, self.motor.ADDR_TORQUE_CONTROL_MODE, TORQUE_ENABLE)
            if (not self.okay(motor_id)):
                self.close(motor_id)
        ...
        for i in range(len(motor_id)):
            dxl_addparam_result = ctypes.c_ubyte(dynamixel.groupSyncWriteAddParam(
                self.group_desTor, motor_id[i], int(des_tor[i]), self.motor.LEN_GOAL_TORQUE)).value
            if dxl_addparam_result != 1:
                print(dxl_addparam_result)
                print("[ID:%03d] groupSyncWrite addparam failed" %
                      (motor_id[i]))
    ...
    def setIndividual_des_torque(self, motor_id, des_tor):
        ...
        for i in range(len(motor_id)):
            dynamixel.write2ByteTxRx(
                self.port_num, self.protocol, motor_id[i], self.motor.ADDR_GOAL_TORQUE, int(des_tor[i]))
        if (not self.okay(motor_id)):
            self.close(motor_id)
    ...
    def set_max_vel(self, motor_id, max_vel):
        for dxl_id in motor_id:
            dynamixel.write4ByteTxRx(
                self.port_num, self.protocol, dxl_id, self.motor.ADDR_MAX_VELOCITY, max_vel)
            if (not self.okay(motor_id)):
                self.close(motor_id)
...
def main(motor_id, motor_type, device, baudrate, protocol):
    ...
    dxl_goal_position = [DXL_MINIMUM_POSITION_VALUE*POS_SCALE,
                         DXL_MAXIMUM_POSITION_VALUE*POS_SCALE]         # Goal position
    index = 0
    ...
    # Test timing ==================================
    cnt = 10
    print("Testing timings (avg over %d trials for %d motors) -------------" %
          (cnt, len(dxl_ids)))
    t_s = time.time()
    for i in range(cnt):
        dxl_present_position, dxl_present_velocity = dy.get_pos_vel(dxl_ids)
    t_e = time.time()
    print("Bulk [pos+vel] read takes:\t\t %0.5f sec **BEST**" %
          ((t_e-t_s)/float(cnt)))
    ...
    t_e = time.time()
    print("Individual [pos,vel] read takes:\t %0.5f sec" %
          ((t_e-t_s)/float(cnt)))
    ...
    t_e = time.time()
    print("Bulk [pos+vel,ctrl] loop takes:\t\t %0.5f sec **BEST**" %
          ((t_e-t_s)/float(cnt)))
    ...
    for i in range(cnt):
        ...
        dy.set_des_pos(dxl_ids, dxl_present_position)
    t_e = time.time()
    print("Bulk [pos,vel,ctrl] loop takes:\t\t %0.5f sec" %
          ((t_e-t_s)/float(cnt)))
    ...
    t_e = time.time()
    print("Individual [pos,vel,ctrl] takes:\t %0.5f sec" %
          ((t_e-t_s)/float(cnt)))
    ...
    for i in range(cnt):
        ...
        for j in range(len(dxl_ids)):
            print("cnt:%03d, dxl_id:%01d ==> Pos:%2.2f, Vel:%1.3f" % (
                i, dxl_ids[j], dxl_present_position[j], dxl_present_velocity[j]))
    ...
    while 1:
        ...
        while 1:
            ...
            for j in range(len(dxl_ids)):
                print("cnt:%03d, dxl_id:%01d ==> GoalPos:%2.2f, Pos:%2.2f, Vel:%1.3f" % (
                    cnt, dxl_ids[j], dxl_goal_position[index], dxl_present_position[j], dxl_present_velocity[j]))

            cnt = cnt + 1
            # checking only the first motor
            if not (abs(dxl_goal_position[index] - dxl_present_position[0]) > DXL_MOVING_STATUS_THRESHOLD):
                break
